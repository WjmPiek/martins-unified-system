"""Fix Gross Old royalty method normalization and recalculate royalties

Revision ID: v67_gross_old_royalty_fix
Revises: v66_performance_engine
Create Date: 2026-06-29
"""
from alembic import op

revision = "v67_gross_old_royalty_fix"
down_revision = "v66_performance_engine"
branch_labels = None
depends_on = None
...
def upgrade():
    ...
    SET royalty_gross_method = CASE
        WHEN lower(regexp_replace(COALESCE(royalty_gross_method, ''), '[^a-zA-Z0-9]+', ' ', 'g')) LIKE '%new%' THEN 'new'
        WHEN lower(regexp_replace(COALESCE(royalty_gross_method, ''), '[^a-zA-Z0-9]+', ' ', 'g')) LIKE '%old%' THEN 'old'
    ...
    WITH bases AS (
        SELECT
            ...
            COALESCE(mf.insurance_payover,0) AS insurance_payover_calc,
        ...
        SELECT
            ...
            COALESCE((
                ...
                SELECT rs2.percentage
                FROM royalty_scales rs2
                WHERE rs2.franchise_id = c.franchise_id
                  AND c.royalty_base >= COALESCE(rs2.amount_from,0)
                ORDER BY COALESCE(NULLIF(rs2.amount_to,0), 999999999999) DESC, rs2.row_number DESC, rs2.id DESC
                LIMIT 1
            ),0)) AS percentage
        FROM calc c
    )
    ...
    SET
        ...
        payover = pct.insurance_payover_calc,
